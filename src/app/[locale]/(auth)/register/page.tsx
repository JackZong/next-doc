...
function RegisterForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/login?redirect=${encodeURIComponent(redirect)}&email=${encodeURIComponent(email)}`);
    } catch {
  ...
  return (
    <Card className="w-full max-w-md bg-card border-border backdrop-blur-xl shadow-xl">
      <CardHeader className="space-y-1 text-center">
        ...
        <CardTitle className="text-2xl font-bold text-foreground">{t('createAccount')}</CardTitle>
      ...
      <form onSubmit={handleSubmit}>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="name" className="text-foreground/80">{t('name')}</Label>
            <Input
              id="name"
              type="text"
          ...
          <div className="space-y-2">
            <Label htmlFor="confirmPassword" className="text-foreground/80">{t('confirmPassword')}</Label>
            <PasswordInput
              id="confirmPassword"
